...
class ButlerRobot(Node):
    ...
    def process_multi_table_order(self, table_nums):
        ...
        for table_num in valid_tables:
            ...
            table_response = self.wait_at_location(10, f"Confirm delivery at Table {table_num}? (type 'confirm' or 'cancel')")
            
            if table_response != 'confirm':
                print(f"No confirmation at Table {table_num}, continuing with next table")
            else:
                print(f"Delivery confirmed at Table {table_num}")
        
        # Return to kitchen after all deliveries
        self.get_logger().info('All deliveries completed. Returning to kitchen.')
        result = self.navigate_to('KITCHEN', 'Kitchen')
        
        # Wait at kitchen for 10 seconds
        self.wait_at_location(10, "Waiting at kitchen before returning home...")
